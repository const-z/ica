...
Запустить ica-ingine:  

```bash
cargo run -p ica-engine
```

Сгенерировать схему из ~10 узлов:

```bash
cargo run --bin generate_schema 10
```

Запустить имитатор поставщика инцидентов: 

```bash
cargo run --bin incident_provider schema_id 2 3
```

Запустить клиента для наблюдения за схемой:
